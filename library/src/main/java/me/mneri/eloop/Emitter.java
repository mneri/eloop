...
public class Emitter {
    // Event-callback mapping. The client can register many callbacks for each event, hence every event is mapped to
    // a list of callbacks. We leave the list of raw type Callback (and not Callback<T>) since two events can require
    // two different types of Callback.
     ...
     */
    public <T> Emitter off(String event, Callback<T> callback) {
     ...
     */
    public <T> Emitter on(String event, Callback<T> callback) {
        if (!isEventLoopThread())
     ...
     * @return This {@code Emitter} so the calls can be chained.
     */
    public <T> Emitter on(String[] events, Callback<T> callback) {
        if (!isEventLoopThread())
            throw new NotOnEventLoopThreadException("on(String[], Callback)");
     ...
     * @return This {@code Emitter} so the calls can be chained.
     */
    public <T> Emitter once(String[] events, Callback<T> callback) {
        if (!isEventLoopThread())
            throw new NotOnEventLoopThreadException("once(String[], Callback)");
    ...
    /**
     * Run the specified {@link Callback}.
     *
     * @param callback The callback to run.
     * @param event    The event the callback is bound to.
     * @param data     The data to pass to the callback.
     */
    @SuppressWarnings({"unchecked", "unsafe"})
    private void runCallback(Callback callback, String event, Object data) {
        try {
